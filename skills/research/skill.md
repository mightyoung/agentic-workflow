...
- [ ] 已使用 tavily/websearch/baidu-search 完成搜索
- [ ] 搜索结果已分析并提取关键要点
- [ ] findings.md 已创建并包含结构化研究结果
- [ ] 研究结果已准备好看 THINING/PLANNING 阶段使用
...
**搜索工具优先级**：
1. **优先使用 tavily skill**：`skill("tavily", "搜索内容")`
   - tavily 是 AI 优化的搜索，返回简洁相关内容
2. **降级使用 websearch**：当 tavily 不可用或失败时
3. **最终降级使用百度搜索**：`skills/baidu-search/skill.md`
   - 当 tavily 和 websearch 都不可用时使用百度AI搜索

**搜索执行**：

```bash
# 优先 tavily 搜索
skill("tavily", "分布式事务最佳实践")

# 降级 websearch
WebSearch(query="distributed transaction best practices", num_results=10)

# 最终降级百度搜索
# 使用 skills/baidu-search/skill.md 中的 baidu_search() 函数
```
