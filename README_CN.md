...
- `validate` 现在会同时检查 state schema 与 state/sidecar 一致性。
...
- self-improvement 线只执行受控改进，并记录 evidence/proposal，以及 proposal 生命周期索引（`knowledge/skill_proposals/index.jsonl`）。
- skill 演化走 proposal 流程（`scripts/skill_evolution.py`），不是自动改写技能文档。
- proposal 校验门（`scripts/proposal_verifier.py`）执行 `approve/revise/reject` 决策，并使用阈值配置（`configs/proposal_verifier.toml`）。
