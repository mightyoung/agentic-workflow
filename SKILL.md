...
description: >
  统一智能体工作流 - 用于任何复杂任务开发。
  触发条件：当用户请求开发功能、修复bug、规划项目、代码审查、
  技术调研、实现需求时自动激活。包含思考分析、任务规划、代码执行、
  规范审查、调试等完整流程。不处理闲聊、天气等日常问题。
  核心方法：TDD驱动、专家模拟思维、文件持久化、PUA激励。
  v4.1: 增强隐式意图识别 + 意图强度分层 + 7个子智能体
---
...
if 包含("帮我搜索", "查找最佳实践", "调研一下", "调研", "搜索一下"):
    → RESEARCH → THINKING
...
# ============================================================================
# 第3层：隐式意图识别（低置信度 - 间接表达）
# ============================================================================

# 隐式 DEBUGGING - 问题描述
elif 包含("响应很慢", "太慢了", "跑不通", "不能用", "失效", "超时", "卡死", "无响应"):
    → DEBUGGING + performance_expert

# 隐式 THINKING - 分析需求
elif 包含("这个怎么实现", "那个行不行", "哪个好", "建议", "看法", "思路"):
    → THINKING

# 隐式 PLANNING - 整理需求
elif 包含("要做什么", "步骤", "先后顺序", "先做哪个"):
    → PLANNING
...
# 闲聊不触发
elif 包含("天气", "笑话", "你好", "谢谢", " hi", "hello") and not 包含("开发", "代码", "帮我"):
    → 直接回答
